...
public class StableMarriageService {
    ...
    public List<Match> executarCasamentoEstavel() {
        ...
        Map<Long, Vaga> matchVaga = new HashMap<>();
        Map<Long, Candidato> matchCandidato = new HashMap<>();
        ...
        while (!candidatosLivres.isEmpty()) {
            ...
            if (!matchVaga.containsKey(vaga.getId())) {
                // Vaga está livre, aceita a proposta
                matchVaga.put(vaga.getId(), vaga);
                matchCandidato.put(candidato.getId(), candidato);
            } else {
                // Vaga já está emparelhada, precisa decidir
                Candidato candidatoAtual = matchCandidato.get(vaga.getId());
                ...
                if (scoreNovoCandidato > scoreCandidatoAtual) {
                    ...
                    Candidato candidatoAnterior = candidatoAtual;
                    matchCandidato.remove(candidatoAnterior.getId());
                    candidatosLivres.add(candidatoAnterior);
        ...
        for (Map.Entry<Long, Vaga> entry : matchVaga.entrySet()) {
            Vaga vaga = entry.getValue();
            Candidato candidato = matchCandidato.get(vaga.getId());
